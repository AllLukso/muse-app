...
export class CustomInjectedConnector extends AbstractConnector {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    ...
    let account
    try {
      account = await (window.ethereum.request as SendOld)({ method: 'eth_accounts' }).then(
    ...
    if (!account) {
      try {
        account = await (window.ethereum.request as SendOld)({ method: 'eth_requestAccounts' }).then(
          sendReturn => parseSendReturn(sendReturn)[0]
        )
      } catch (error) {
        if ((error as any).code === 4001) {
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId
    try {
      chainId = await (window.ethereum.request as SendOld)({ method: 'eth_chainId' }).then(parseSendReturn)
    } catch {
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.ethereum.request as SendOld)({ method: 'eth_accounts' }).then(sendReturn => parseSendReturn(sendReturn)[0])
    } catch {
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.ethereum.request as SendOld)({ method: 'eth_accounts' }).then(sendReturn => {
